...
public class XmlPullParserHelper {
     ...
     */
    public static void throwIfUnknownTag(XmlPullParser parser)
            throws XmlPullParserException, IOException {
        if (parser.getEventType() != XmlPullParser.START_TAG) throw new IllegalStateException();

        throw new XmlPullParserException(
                "Unknown or unhandled XML tag: <" + parser.getName() + ">. "
                        + "Please check if the tag is spelled correctly and its parser "
                        + "is registered in CoreParserModule.");
    }
     ...
     * the text from the subsequent TEXT event and consumes the corresponding END_TAG to advance the
     * parser.
     *
