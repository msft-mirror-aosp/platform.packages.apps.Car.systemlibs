...
import com.android.car.scalableui.designcompose.ScalableUiDoc;
import com.android.car.scalableui.designcompose.ScalableUiDocKt;
...
public final class PanelStateDocLoader {
  ...
  private PanelState createPanelStateFromDoc(
    ...
    // Add variants
    Log.i(TAG, "createPanel " + setData.getName() + ", default " + setData.getDefaultVariantName());
    int index = 0;
  ...
  private Variant createVariantFromDoc(
    ...
    Variant result =
        new Variant.Builder(variant.getName())
            .setLayer(new Layer.Builder().setLayer(variant.getLayer()).build().getLayer())
            .setVisibility(
                new Visibility.Builder().setIsVisible(variant.getIsVisible()).build().isVisible())
  ...
  private Bounds createBoundsFromDoc(@NonNull ScalableUiVariant scalableUiVariant) {
    ...
    return new Bounds.Builder().setLeft(left).setTop(top).setWidth(width).setHeight(height).build();
  }
  ...
  private Transition createTransitionFromDoc(
    ...
    if (toVariant == null) {
      throw new DocLoadException("Could not find variant with name " + event.getToVariantName());
    }
